        ...
        if (client != null && client.world != null && config.isEnabled()) { // Solo renderizar si está activado
            ...
            for (int i = 0; i < displayCount; i++) {
                ...
                if (config.isShowBackground()) {
                    InGameHud.fill(matrixStack, backgroundX, backgroundY, backgroundX + backgroundWidth, backgroundY + backgroundHeight, backgroundColor);
                }

                // Dibujar el texto
                client.textRenderer.draw(matrixStack, timeLeft, 0, 0, color);
                matrixStack.pop();
            }
        }
    }
