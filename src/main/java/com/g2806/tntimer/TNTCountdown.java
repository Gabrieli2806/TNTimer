...
public class TNTCountdown implements ClientModInitializer {
    public static final String MOD_ID = "tntimer"; // Added from TNTimer.java
    public static final Logger LOGGER = LoggerFactory.getLogger(MOD_ID); // Updated to use MOD_ID
    ...
    public void onInitializeClient() {
        ...
        LOGGER.info("Hello Fabric world!"); // Added from TNTimer.java
        ...
        configKey = KeyBindingHelper.registerKeyBinding(new KeyBinding(
                "Open TNT Countdown Config", // Hardcoded English string
                InputUtil.Type.KEYSYM,
                InputUtil.GLFW_KEY_K,
                "TNT Countdown" // Hardcoded English category
        ));
    }
        ...
        if (client != null && client.world != null && config.isEnabled()) { // Solo renderizar si está activado
            ...
            for (int i = 0; i < displayCount; i++) {
                ...
                if (config.isShowOnlySeconds()) {
                    ...
                    timeLeft = "TNT explosion in " + formattedSeconds + "s"; // Hardcoded English string
                }
                ...
                if (config.isShowBackground()) {
                    client.textRenderer.drawWithShadow(matrixStack, "", backgroundX, backgroundY, 0); // Dummy draw to apply matrix
                    client.inGameHud.getTextRenderer().draw(matrixStack, "", backgroundX, backgroundY, backgroundColor);
                    // Usar fill para el fondo (en 1.18.2 no hay DrawContext.fill)
                    net.minecraft.client.gui.DrawableHelper.fill(matrixStack, backgroundX, backgroundY, backgroundX + backgroundWidth, backgroundY + backgroundHeight, backgroundColor);
                }

                // Dibujar el texto
                client.textRenderer.drawWithShadow(matrixStack, timeLeft, 0, 0, color);
                matrixStack.pop();
            }
        }
    }
